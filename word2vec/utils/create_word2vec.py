...
def train_gensim_model(file_list,labels_list,word2vec_path,max_sentence_length):
    sentences, batch_labels = files_processing.read_files_labels(file_list, labels_list, max_sentence_length)
    train_wordVectors(sentences, word2vec_path, embedding_size=128, window=5, min_count=5)


def train_wordVectors(sentences, word2vec_path, embedding_size = 128, window = 5, min_count = 5):
...
def save_multi_file(files_list,labels_list,word2vec_path,out_dir,prefix,batchSize,max_sentence_length,labels_set=None,shuffle=False):
    ...
    for i in range(batchNum):
        ...
        # 读取文件内容，字词分割
        batch_content, batch_labels = files_processing.read_files_labels(batch_files, batch_labels, max_sentence_length, one_line=False)
        # 将字词转为索引矩阵
...
def pos_neg_test():
    ...
    sentences, max_document_length = files_processing.padding_sentences(sentences, '<PADDING>', padding_sentence_length=190)
...
if __name__=='__main__':
    ...
    # 利用gensim训练一个word2vec模型
    max_sentence_length=300
    word2vec_path="out/THUCNews_word2vec300.model"
    # train_gensim_model(files_list, label_list, word2vec_path,max_sentence_length)
